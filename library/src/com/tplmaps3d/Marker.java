...
import com.tplmaps3d.geometry.Polygon;
...
public class Marker {
    ...
    public int getLastVisitedIndexOfGeometryMapper(){
        return map.getMarkerLastVisitedIndexOfGeometryMapper();
    }

    public void setPointEasedUsingGeometryMapper(LngLat point, float duration, int easeType, boolean reversed){

        map.markerSetPointEasedUsingGeometryMapper(markerId,point,duration,easeType,reversed);
    }

    public void setPointEasedUsingGeometryMapper(LngLat point, int bufferStartIndex, int bufferEndIndex, float duration, int easeType, boolean reversed){
        map.markerSetPointEasedUsingGeometryMapper(markerId,point,bufferStartIndex,bufferEndIndex,duration,easeType,reversed);
    }

    public double distanceFromEndOfCurrentBufferOfGeometryMapper(){
        return map.markerDistanceFromEndOfCurrentBufferOfGeometryMapper();
    }

    public void setRotation(double radians){
        map.markerSetRotation(markerId,radians);
    }

    public void setRotationEased(double radians,float duration, int easeType) {
        map.markerSetRotationEased(markerId,radians,duration,easeType);
    }

    public float getRotation(){
        return map.markerGetRotation(markerId);
    }

    public boolean lockWithViewPort(boolean lockPos, boolean lockRot){
        return map.markerLockWithViewPort(markerId,lockPos,lockRot);
    }

    public boolean markerUnlockWithViewPort(){

        return map.markerUnlockWithViewPort(markerId);
    }
